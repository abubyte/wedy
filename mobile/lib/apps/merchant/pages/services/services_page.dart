      ...
      body: BlocConsumer<MerchantServiceBloc, MerchantServiceState>(
        ...
        builder: (context, state) {
          if (state is MerchantServiceLoading &&
              state.type == MerchantServiceLoadingType.initial &&
              state.previousData == null) {
            return const Center(child: CircularProgressIndicator());
          } else if (state is MerchantServiceInitial) {
            return const Center(child: CircularProgressIndicator());
          } else if (state is MerchantServiceLoaded) {
            ...
            // Show error with retry option, but also show previous data if available
            if (state.previousData != null && state.previousData!.hasServices) {
              return RefreshIndicator(
                ...
                child: ListView.builder(
                  padding: const EdgeInsets.all(AppDimensions.spacingL),
                  itemCount: state.previousData!.services.length,
                  itemBuilder: (context, index) {
                    final service = state.previousData!.services[index];
                    return _ServiceCard(
                      service: service,
                      onTap: () => context.pushNamed(RouteNames.edit, extra: service),
                      onDelete: () => _showDeleteDialog(context, service),
          ...
          } else if (state is MerchantServiceLoading && state.previousData != null) {
            // Show loading indicator over previous data
            return Stack(
              children: [
                RefreshIndicator(
                  onRefresh: () async {
                ...
                if (state.type != MerchantServiceLoadingType.initial)
                  Container(
