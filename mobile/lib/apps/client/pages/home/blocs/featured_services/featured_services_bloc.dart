...
class FeaturedServicesBloc extends Bloc<FeaturedServicesEvent, FeaturedServicesState> {
  ...
  FeaturedServicesBloc({required GetFeaturedServices getFeaturedServicesUseCase})
  ...
  Future<void> _fetchFeaturedServices(FetchFeaturedServices event, Emitter<FeaturedServicesState> emit) async {
    emit(state.copyWith(status: StateStatus.loading));

    final result = await _getFeaturedServicesUseCase();

    result.fold(
      (failure) => emit(state.copyWith(status: StateStatus.error, message: "Qaynoq takliflarni yuklab bo'lmadi")),
      (response) => emit(
        state.copyWith(
